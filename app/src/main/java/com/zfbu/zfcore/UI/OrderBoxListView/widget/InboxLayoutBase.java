package com.zfbu.zfcore.UI.OrderBoxListView.widget;
...
public abstract class InboxLayoutBase<T extends View> extends FrameLayout {
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            if (mStartTime == -1) {
                mStartTime = Core.getStamp();
            } else {
                long normalizedTime = (1000 * (Core.getStamp() - mStartTime)) / mDuration;
    ...
    public void closeWithAnim() {
        Config.order_page_list_hasopen = false; /////订单列表是否关闭
        topView.setAlpha(1);
